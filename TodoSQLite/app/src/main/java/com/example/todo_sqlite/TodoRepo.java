...
public class TodoRepo extends SQLiteOpenHelper {
    ...
    public static final String SQL_CREATE_TABLE = "CREATE TABLE " + TABLE_NAME + " (" + COLUMN_ID + " TEXT PRIMARY KEY, " + COLUMN_TITLE + " TEXT, " + COLUMN_DESC + " TEXT, " + COLUMN_IS_DONE + " INTEGER " + ")";
    ...
    public void addNew(TodoItem item) {
        ...
        values.put(this.COLUMN_ID, item.getId());
        values.put(COLUMN_TITLE, item.getTitle());
        values.put(COLUMN_DESC, item.getDesc());
    ...
    public boolean update(TodoItem item) {
        ...
        values.put(COLUMN_DESC, item.getDesc());
        values.put(COLUMN_IS_DONE, item.isDone() ? 1 : 0);
        int rowAffected = db.update(TABLE_NAME, values, COLUMN_ID + "= ?", new String[]{item.getId()});
        db.close();
    ...
    public ArrayList<TodoItem> loadAll() {
        ...
        while (cursor.moveToNext()) {
            ...
            items.add(new TodoItem(id, title, desc,isDone));
        }
        db.close();
        return items;
    }
    ...
    public TodoItem getById(String id){
       ...
       if(cursor.moveToFirst()){
           ...
           Boolean isDone = cursor.getInt(3)==1;
           db.close();
           return new TodoItem(id, title, desc,isDone);
       }
       return null;
    }
